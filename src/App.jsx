...
// Datos iniciales (para modo offline o primera carga)
...
export default function CucharelliApp() {
  ...
  const [stockViewMode, setStockViewMode] = useState('main'); // 'main' (Vitrina) o 'daniela' (Casa Daniela)
  const [reportFilter, setReportFilter] = useState('month'); // 'all', 'month', 'today'

  // Formularios
  const [newProdName, setNewProdName] = useState('');
  const [newProdImage, setNewProdImage] = useState(null);
  const [editingId, setEditingId] = useState(null);
  const [stockInputs, setStockInputs] = useState({}); // Para producción
  const [transferInputs, setTransferInputs] = useState({}); // Para enviar a Daniela
  ...
  // Edición de Transacciones (Reportes)
  ...
  useEffect(() => {
    ...
    const unsubProd = onSnapshot(qProd, (snapshot) => {
      setProducts(snapshot.docs.map(doc => ({ 
        id: doc.id, 
        ...doc.data(),
        // Aseguramos que existan los campos aunque sean viejos registros
  ...
  const handleSaveProduct = async () => {
    ...
    if (editingId) {
      if (isFirebaseReady && db) {
        const prodRef = doc(db, 'artifacts', appId, 'public', 'data', 'products', editingId);
  ...
  const updateStockBulk = async () => {
    ...
    if (isFirebaseReady && db) {
      ...
      updates.forEach(([id, qty]) => {
        ...
        batch.update(ref, { stock: increment(parseInt(qty)) });
      });
      const prodRef = doc(collection(db, 'artifacts', appId, 'public', 'data', 'transactions'));
      batch.set(prodRef, {
        type: 'restock',
        location: 'main',
        items: updates.map(([id, qty]) => {
          const p = products.find(x => x.id === id);
          return { name: p?.name, qty: parseInt(qty) };
        }),
    ...
    setStockInputs({});
    alert("Producción registrada en Vitrina");
  };

  // 2. Enviar a Daniela (Resta Vitrina -> Suma Daniela)
  const transferToDaniela = async () => {
    ...
    for (let [id, qty] of updates) {
      ...
      if (prod.stock < parseInt(qty)) return alert(`No tienes suficiente ${prod.name} en vitrina para enviar.`);
    }
    ...
    if (isFirebaseReady && db) {
      ...
      updates.forEach(([id, qty]) => {
        const ref = doc(db, 'artifacts', appId, 'public', 'data', 'products', id);
        // Resta de vitrina, suma a Daniela
        batch.update(ref, { 
          stock: increment(-parseInt(qty)),
          stockDaniela: increment(parseInt(qty))
        });
      });
      ...
      batch.set(transRef, {
        type: 'transfer',
        description: 'Envío a Casa de Daniela',
        amount: 0, // No es venta, no hay dinero
        items: updates.map(([id, qty]) => {
          const p = products.find(x => x.id === id);
          return { name: p?.name, qty: parseInt(qty) };
        }),
    ...
    setTransferInputs({});
    alert("✅ Enviado a Casa de Daniela correctamente");
  };
  ...
  const registerMultiSale = async () => {
    ...
    if (isFirebaseReady && db) {
      ...
      const transRef = doc(collection(db, 'artifacts', appId, 'public', 'data', 'transactions'));
      batch.set(transRef, {
        type: 'sale', amount: totalSaleAmount,
  ...
  const registerMultiExpense = async () => {
    ...
    if (validRows.length === 0) return alert("Añade un gasto válido");
    const totalExpense = validRows.reduce((acc, r) => acc + parseFloat(r.amount), 0);
    if (isFirebaseReady && db) {
    ...
    setExpenseRows([{ id: Date.now(), desc: '', amount: '' }]);
  ...
  const deleteTransaction = async (id) => {
    if (!window.confirm("¿Borrar este movimiento? Esto afectará el cálculo final de la caja.")) return;
    if (isFirebaseReady && db) {
  ...
  const exportToCSV = () => {
    const headers = ["Fecha,Tipo,Descripcion,Monto"];
    const rows = filteredTransactions.map(t => {
      const date = t.createdAt.toLocaleDateString();
      const type = t.type === 'sale' ? 'Venta' : t.type === 'expense' ? 'Gasto' : 'Movimiento';
      const desc = t.description || (t.items ? t.items.map(i => `${i.qty || ''} ${i.name || i.desc}`).join(' + ') : '');
      return `${date},${type},"${desc}",${t.amount || 0}`;
    });
    const csvContent = "data:text/csv;charset=utf-8," + headers.concat(rows).join("\n");
    const encodedUri = encodeURI(csvContent);
    const link = document.createElement("a");
    link.setAttribute("href", encodedUri);
    link.setAttribute("download", `reporte_cucharelli_${new Date().toISOString().slice(0,10)}.csv`);
        ...
        return (
          <div className="p-4 space-y-4 pb-20">
            <div className="flex justify-between items-center mb-2">
              <h2 className={`text-2xl font-bold ${BRAND.brownText} flex items-center gap-2`}>
                <Package className="text-[#E91E63]" /> 
                {stockViewMode === 'main' ? 'Tu Vitrina' : 'Casa Daniela'}
              </h2>
              {/* INTERRUPTOR DE VISTA */}
              <div className="bg-gray-200 p-1 rounded-lg flex text-xs font-bold">
                <button 
                  onClick={() => setStockViewMode('main')}
                  className={`px-3 py-1 rounded-md transition-all ${stockViewMode === 'main' ? 'bg-white shadow text-[#5D4037]' : 'text-gray-400'}`}
                >
                  Vitrina
                </button>
                <button 
                  onClick={() => setStockViewMode('daniela')}
                  className={`px-3 py-1 rounded-md transition-all ${stockViewMode === 'daniela' ? 'bg-white shadow text-[#E91E63]' : 'text-gray-400'}`}
                >
                  Daniela
                </button>
              </div>
                ...
                return (
                  <div key={p.id} className="bg-white rounded-xl shadow-sm border border-[#EFEBE9] overflow-hidden flex flex-col group">
                    <div className="h-32 w-full relative bg-gray-100 flex items-center justify-center">
                      ...
                      <div className={`absolute top-2 right-2 px-2 py-0.5 rounded-full text-xs font-bold shadow ${stockViewMode === 'main' ? 'bg-white text-[#5D4037]' : 'bg-[#E91E63] text-white'}`}>
                        x{currentStock}
                      </div>
                    </div>
            ...
            {products.length === 0 && <div className="text-center p-8 text-gray-400">Aún no hay sabores registrados. Ve a Ajustes.</div>}
          </div>
        );
      ...
      case 'add_stock':
        return (
          <div className="p-4 pb-24 space-y-8">
            {/* SECCIÓN 1: PRODUCCIÓN VITRINA */}
            <div>
              <h2 className={`text-2xl font-bold ${BRAND.brownText} mb-2 flex items-center gap-2`}><PlusCircle className="text-[#E91E63]" /> Producción (Vitrina)</h2>
              <p className="text-sm text-[#8D6E63] mb-4">¿Qué prepararon hoy?</p>
                  ...
                  <div key={p.id} className="flex items-center justify-between bg-white p-3 rounded-xl shadow-sm border border-[#EFEBE9]">
                    <div className="flex items-center gap-3 overflow-hidden">
                      <div className="w-10 h-10 rounded-lg bg-gray-100 flex-shrink-0 overflow-hidden">
                        {p.photo ? <img src={p.photo} className="w-full h-full object-cover" /> : <div className="w-full h-full bg-[#5D4037]/10" />}
                      </div>
                      <span className={`font-bold ${BRAND.brownText} truncate text-sm`}>{p.name}</span>
                    </div>
                    <input type="number" placeholder="0" className="w-12 bg-[#F9F7F2] rounded p-1 text-center font-bold text-[#E91E63] outline-none border border-transparent focus:border-[#E91E63]"
                        value={stockInputs[p.id] || ''} onChange={(e) => setStockInputs({...stockInputs, [p.id]: e.target.value})} />
                  </div>
                ))}
              </div>
              <button onClick={updateStockBulk} className={`mt-4 ${BRAND.brown} text-white w-full py-3 rounded-xl font-bold shadow-lg flex justify-center gap-2`}><Save size={20} /> GUARDAR STOCK</button>
            </div>

            {/* SECCIÓN 2: ENVIAR A DANIELA */}
            <div className="bg-[#E3F2FD] p-5 rounded-2xl border border-blue-100">
              <h2 className={`text-xl font-bold text-blue-800 mb-2 flex items-center gap-2`}><Home size={20} /> Enviar a Casa de Daniela</h2>
              <p className="text-xs text-blue-600 mb-4">Mueve productos de tu Vitrina hacia Daniela.</p>
              <div className="space-y-3">
                {products.map(p => (
                  <div key={p.id} className="flex items-center justify-between bg-white/50 p-2 rounded-lg border border-blue-100">
                     <span className="text-sm text-blue-900 font-medium truncate w-1/2">{p.name}</span>
                     <div className="flex items-center gap-2">
                       <span className="text-[10px] text-gray-400">Vitrina: {p.stock}</span>
                       <input type="number" placeholder="0" className="w-12 bg-white rounded p-1 text-center font-bold text-blue-600 outline-none border border-blue-200 focus:border-blue-500"
                          value={transferInputs[p.id] || ''} onChange={(e) => setTransferInputs({...transferInputs, [p.id]: e.target.value})} />
                     </div>
                  </div>
                ))}
              </div>
              <button onClick={transferToDaniela} className={`mt-4 bg-blue-600 text-white w-full py-3 rounded-xl font-bold shadow-lg flex justify-center gap-2`}><ArrowRightLeft size={20} /> TRANSFERIR</button>
            </div>
          </div>
        );
      ...
      case 'sales':
        return (
          <div className="p-4 pb-24 max-w-lg mx-auto">
            <h2 className={`text-2xl font-bold ${BRAND.brownText} mb-2 flex items-center gap-2`}><ShoppingCart className="text-[#E91E63]" /> Nueva Venta</h2>
            <div className="space-y-3 mb-20">
              ...
              {saleRows.map((row) => (
                <div key={row.id} className="bg-white p-2 rounded-xl shadow-sm border border-[#EFEBE9] flex gap-2 items-center animate-in fade-in slide-in-from-bottom-2">
                  <div className="flex-1">
                    ...
                    {row.prodId && <div className="text-[9px] text-gray-400 ml-1 mt-0.5">Stock: {products.find(p => p.id === row.prodId)?.stock || 0}</div>}
                  </div>
              ...
              <button onClick={addSaleRow} className="w-full py-3 border-2 border-dashed border-[#D7CCC8] rounded-xl text-[#8D6E63] font-bold text-sm flex justify-center items-center gap-2 hover:bg-[#EFEBE9]"><Plus size={18} /> Agregar producto</button>
            </div>
            <div className="fixed bottom-20 left-0 right-0 p-4 bg-white border-t border-gray-100 shadow-[0_-5px_20px_rgba(0,0,0,0.05)]">
              ...
              <button onClick={registerMultiSale} className="w-full bg-[#4CAF50] text-white py-3 rounded-xl font-bold shadow-lg">CONFIRMAR VENTA</button>
            </div>
          </div>
        );
      ...
      case 'expenses':
        return (
          <div className="p-4 pb-24">
             <h2 className={`text-2xl font-bold ${BRAND.brownText} mb-2 flex items-center gap-2`}><TrendingUp className="text-red-500" /> Registrar Gastos</h2>
            <div className="space-y-3 mb-20">
              {expenseRows.map((row) => (
        ...
        return (
          <div className="p-4 pb-20">
            <div className="flex justify-between items-center mb-6">
              <h2 className={`text-2xl font-bold ${BRAND.brownText} flex items-center gap-2`}><Activity className="text-[#E91E63]" /> Reporte</h2>
              <div className="flex gap-2">
                <button onClick={exportToCSV} className="bg-green-100 text-green-700 p-2 rounded-lg text-xs font-bold flex items-center gap-1"><FileDown size={16}/> Excel</button>
            ...
            <div className="space-y-3">
              {filteredTransactions.map(t => (
                <div key={t.id} className={`bg-white p-3 rounded-xl shadow-sm border ${editingTransId === t.id ? 'border-blue-400 bg-blue-50' : 'border-[#EFEBE9]'}`}>
                  {editingTransId === t.id ? (
                    // MODO EDICIÓN DE TRANSACCIÓN
                    <div className="flex flex-col gap-2">
                      <p className="text-xs font-bold text-blue-600">Corrigiendo Movimiento...</p>
                      ...
                      <input type="text" className="p-2 border rounded bg-white text-xs" value={editTransDesc} onChange={(e) => setEditTransDesc(e.target.value)} placeholder="Descripción" />
                      <div className="flex gap-2 justify-end mt-1">
                        <button onClick={() => setEditingTransId(null)} className="text-gray-500 text-xs p-1">Cancelar</button>
                        <button onClick={saveTransactionEdit} className="bg-blue-600 text-white text-xs px-3 py-1 rounded font-bold">Guardar</button>
                      </div>
                    </div>
                  ) : (
                    // VISTA NORMAL
                    <>
                      <div className="flex justify-between items-start mb-2">
                        <div className="flex items-center gap-2">
                          ...
                          <span className={`font-bold ${BRAND.brownText} text-sm`}>
                            {t.type === 'sale' ? 'Venta' : t.type === 'expense' ? 'Gasto' : t.type === 'transfer' ? 'Traslado' : 'Producción'}
                          </span>
                        </div>
                        <div className="flex items-center gap-2">
                          <span className="text-[10px] text-gray-400">{t.createdAt.toLocaleDateString()}</span>
                          {/* BOTONES DE EDICIÓN RAPIDA */}
                          <button onClick={() => startEditingTransaction(t)} className="text-blue-400 hover:text-blue-600"><Pencil size={14}/></button>
                          <button onClick={() => deleteTransaction(t.id)} className="text-red-300 hover:text-red-500"><Trash2 size={14}/></button>
                        </div>
                      </div>
                      
                      {t.description && <p className="text-xs font-medium text-gray-600 mb-1">{t.description}</p>}
                      ...
                      <div className="text-right">
                        {t.amount !== undefined && t.amount !== 0 && (
                          <span className={`font-bold ${t.type === 'sale' ? 'text-green-600' : 'text-red-500'}`}>
                            {t.type === 'expense' ? '-' : '+'} S/ {parseFloat(t.amount).toFixed(2)}
                          </span>
                        )}
                      </div>
                    </>
      ...
      case 'settings':
        return (
          <div className="p-4 space-y-6 pb-20">
            <h2 className={`text-2xl font-bold ${BRAND.brownText} flex items-center gap-2`}><Settings className="text-gray-400" /> Ajustes</h2>
            {/* FORMULARIO */}
            <div className={`p-5 rounded-2xl shadow-sm border transition-colors ${editingId ? 'bg-orange-50 border-orange-200' : 'bg-white border-[#EFEBE9]'}`}>
              <h3 className={`font-bold ${editingId ? 'text-orange-700' : BRAND.brownText} mb-4 flex items-center gap-2 justify-between`}>
                <span className="flex items-center gap-2">
                  <ImageIcon size={18} className={editingId ? 'text-orange-500' : 'text-[#E91E63]'}/> {editingId ? 'Editando Producto' : 'Nuevo Sabor'}
                </span>
              ...
              <div className="space-y-3">
                ...
                <div className="relative">
                  ...
                  <div className={`w-full p-3 border-2 border-dashed rounded-xl flex items-center justify-center gap-2 text-sm ${newProdImage ? 'border-[#E91E63] bg-pink-50 text-[#E91E63]' : 'border-gray-300 text-gray-400'}`}>
                    {newProdImage ? <><ImageIcon size={16} /> ¡Nueva Foto Lista!</> : <><Plus size={16} /> {editingId ? 'Cambiar Foto (Opcional)' : 'Subir Foto'}</>}
                  </div>
                </div>
                ...
                <button onClick={handleSaveProduct} className={`${editingId ? 'bg-orange-600 hover:bg-orange-700' : BRAND.brown} text-white w-full py-3 rounded-xl font-bold text-sm shadow transition`}>{editingId ? 'Actualizar Producto' : 'Guardar Producto'}</button>
              </div>
            </div>
            {/* LISTA */}
              ...
              <div className="space-y-3">
                {products.map(p => (
                  <div key={p.id} className="flex items-center justify-between bg-[#F9F7F2] p-2 rounded-xl border border-gray-100">
                    <div className="flex items-center gap-3 overflow-hidden">
                      <div className="w-10 h-10 rounded-lg bg-white flex-shrink-0 overflow-hidden border border-gray-100">
                        {p.photo ? <img src={p.photo} className="w-full h-full object-cover" /> : <div className="w-full h-full bg-[#5D4037]/10" />}
                      </div>
                      <p className={`font-medium ${BRAND.brownText} truncate text-sm max-w-[120px]`}>{p.name}</p>
                    </div>
                    <div className="flex items-center gap-1">
                      <button onClick={() => {setEditingId(p.id); setNewProdName(p.name); window.scrollTo({top:0,behavior:'smooth'});}} className="p-2 text-blue-500 bg-white rounded-lg border border-gray-100 shadow-sm hover:bg-blue-50"><Pencil size={16}/></button>
                      <button onClick={() => handleDeleteProduct(p.id)} className="p-2 text-red-500 bg-white rounded-lg border border-gray-100 shadow-sm hover:bg-red-50"><Trash2 size={16}/></button>
                    </div>
                  </div>
                ))}
              </div>
